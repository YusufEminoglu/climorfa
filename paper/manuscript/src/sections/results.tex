...
The v8 integration completed without row loss: all 16,506 grid identifiers are
...
families show the expected structural absence in cells without the relevant
...
contrasts feature-family missingness with the four-class diagnostic
...
\begin{figure*}[htbp]
    ...
    cell-level plan, LST, imagery, and feature evidence; (d) maps the complete
    569-cell audit queue by review pass; and (e) compares audit-quality flags
    with priority scores. Missingness can be structural and is not automatically
    an error.}
    \label{fig:data-readiness}
\end{figure*}

The manual audit is complete: all 569 rows passed the schema gate, comprising
...
high-quality rows and retains substantial class-space compression.
...
\begin{figure*}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{fig_4_class_profiles.png}
    ...
    means the displayed height-proxy medians use available building-bearing cells.}
...
and audit-compatible/contradictory cells. Probability is not calibrated
confidence, however; the warning score is used as a stability diagnostic rather
...
\begin{figure*}[htbp]
    ...
    least 100 diagnostic rows are included; scores use classes present in each
...
LCZ 9, and LCZ 6. Across cells, NDBI had the strongest positive bivariate
...
high-quality tier, 96 cells were available: exact agreement was 8.3\% for the
weak label, 8.3\% for LightGBM, and 9.4\% for Random Forest. The high-plus-medium
sensitivity tier contained 148 cells, with corresponding agreement of 10.1\%,
12.2\%, and 12.2\%; the all-quality four-class reference contained 165 cells,
with 10.3\%, 13.3\%, and 13.3\%. Figure~\ref{fig:audited-validation} shows
...
\begin{figure*}[htbp]
    ...
    audit-quality tier. The primary tier is the 96-cell high-quality subset;
