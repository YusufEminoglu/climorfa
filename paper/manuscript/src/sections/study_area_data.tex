...
All spatial layers were aligned to a common metric analysis grid. The primary
support is a 250 m regular grid clipped by the FUR boundary. It contains
16,506 grid units, of which 15,801 meet the core boundary-coverage rule. A
500 m grid containing 4,288 units is prepared for scale sensitivity; the 125 m
grid remains a preregistered confirmatory sensitivity for multi-scale analysis.
Industrial, port, airport, water-edge,
and low-coverage units were retained with context and quality flags rather than
silently removed. Figure~\ref{fig:study-area} shows the study frame, the four
weak built classes used in the analysis, and the Landsat LST response.
...
The local surface model was clipped to the FUR at 5 m resolution. Pixels below
...
height. Independent floor-count fields from the municipal building inventory were
...
Municipal building footprints and road centrelines
...
layers for 2020. The accepted export masks artefacts and uses explicit missing
data handling. Aggregated variables include canopy cover above 2, 5, and 10 m,
p95 height, and a volume proxy per hectare. Coast distance was computed from a
quality-checked shoreline layer aligned to the common analysis grid.
...
military/quarry, and construction/brownfield flags. Green-space supply for the 2SFCA uses OSM park and garden
polygons, while 2024 neighbourhood population is dasymetrically allocated to
units in proportion to residential floor-area proxy. Table~\ref{tab:data_inventory}
...
largest within-unit support; confidence is its support share. Labels are
available for 16,233 grid units. At a 0.70 majority threshold, 6,585 units are mixed.
Mixed units are retained as transition or uncertainty candidates, not treated
as automatic errors.
...
class. This retained 5,339 grid units in LCZ 3, 6, 8, and 9. A separate 569-unit
...
review fields, and a three-pass priority queue. The audit comprised 569 records,
all of which passed the audit schema gate. Quality review classified 382 as high, 143 as medium, and 28 as low quality;
16 edge cases also received reject flags. The audit therefore supports a direct
...
census. Table~\ref{tab:analysis_frame} reports the analysis status.
