...
\begin{figure}[!htbp]
    ...
    \caption{CLIMORFA analysis design. (a) Eligible and excluded units in the
    250 m analysis grid. (b) Feature-column availability and leading missingness
    groups. (c) Additive macro-F1 across the model recipes. (d) Density of
    network-access values at 400, 800, and 1,200 m. (e) Spatial folds used for
    model validation.}
    \label{fig:workflow}
\end{figure}
...
To audit supply geography, the 2023-modified Izmir Metropolitan Municipality (IMM) park-maintenance
...
\subsection{CLIMORFA multimodal architecture}

The CLIMORFA architecture has three branches: (i) raster chips for
...
The CLIMORFA evaluation design includes branch-specific ablations and attribution analyses. The ablation set separates the contributions of the surface model, vegetation, network, figure-ground, and remote-semantic inputs; Integrated Gradients, Grad-CAM or occlusion, and tabular SHAP provide complementary interpretation tools. Branch comparisons use the same spatial-validation and audit framework as the tabular analysis.
