...
\subsection{Evidence lock and estimands}

The primary estimand in this version is out-of-sample discrimination of four
high-confidence global LCZ weak labels from local climate-morphology features.
This is a diagnostic of data integration and transferable signal; it is not an
estimate of local audited subtype accuracy. Secondary estimands are (i) the
...
LST. The final intended estimand---audited LCZ-like local subtype prediction---is
now assessed descriptively on the completed audit, but remains limited by the
small high-quality subset and the absence of a trained deep model.

The workflow is summarized in Figure~\ref{fig:workflow}. Every reported result
can be traced to the v8 matrix, a parameter manifest, a processing script, and
an output directory. Planned neural components are visually separated from
completed evidence.
...
The primary grid cell is 250 m. Raster variables were summarized using valid
...
imputation: building and floor variables are structurally absent in cells
...
$S_j$. The 2024 neighbourhood population is allocated to 250 m cells as demand
...
correlation ranges. Name matching is diagnostic only.

\subsection{Weak-label definition and manual audit design}

For cell $i$, the weak label is
...
5-by-5-cell blocks, nominally 1.25 km, and no block appears in both training and
...
A separate leave-one-district-out (LODO) diagnostic tests the LightGBM recipe
...
This is a harder transfer diagnostic, not a replacement for spatial blocks.
...
indicator. The confirmatory model requires calibrated probabilities, SHAP for
tabular components, and spatial attribution for neural branches.

\subsection{Climate-response validation}

LST is never a morphology predictor. For the 5,339-cell diagnostic subset, we
...
The planned confirmatory architecture has three branches: (i) raster chips for
...
are required. The stopping rule is strict: these analyses cannot enter the
...
design. The current paper therefore reports the evidence-locked tabular
baseline and bounded audit diagnostics, not CLIMORFA deep-model performance.
