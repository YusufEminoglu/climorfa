"""Merge the v7 model matrix with network-based green-space 2SFCA metrics."""
...
DEFAULT_INPUTS = [
    "data/03_processed/grid_250m_model_features_v7.csv",
    "data/03_processed/grid_250m_green_space_2sfca.csv",
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-csv", default="data/03_processed/grid_250m_model_features_v8.csv")
...
def read_csv_by_grid_id(path: str) -> tuple[list[str], dict[str, dict[str, str]]]:
    with Path(path).open(newline="", encoding="utf-8") as handle:
        ...
        if reader.fieldnames is None or "grid_id" not in reader.fieldnames:
...
def main() -> None:
    ...
    for input_index, path in enumerate(args.inputs):
        ...
        for grid_id in base_ids:
            source_row = rows.get(grid_id)
            if source_row is None:
                missing += 1
                continue
            for field in fields:
                if field != "grid_id":
                    merged[grid_id][field] = source_row.get(field, "")
