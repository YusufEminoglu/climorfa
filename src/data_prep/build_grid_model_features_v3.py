"""Merge v2 model features with exact road/network grid metrics."""
...
DEFAULT_INPUTS = [
    "data/03_processed/grid_250m_model_features_v2.csv",
    "data/03_processed/grid_250m_road_network_features.csv",
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-csv", default="data/03_processed/grid_250m_model_features_v3.csv")
...
def add_derived_flags(row: dict[str, str]) -> None:
    row["road_presence_flag"] = "1" if as_float(row.get("road_length_exact_m")) > 0 else "0"
    row["network_intersection_presence_flag"] = (
        "1" if as_float(row.get("network_intersection_node_count")) > 0 else "0"
    )
...
def main() -> None:
    ...
    for derived in ["road_presence_flag", "network_intersection_presence_flag"]:
